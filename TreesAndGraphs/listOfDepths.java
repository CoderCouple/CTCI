...
public class BinaryTree {
	...
	public static TNode getMinimalTree(int[] array, int start, int end) {
		if (start > end)
			return null;

		int mid = (start + end) / 2;
		TNode node = new TNode(array[mid]);
		node.leftChild = getMinimalTree(array, start, mid - 1);
		node.rightChild = getMinimalTree(array, mid + 1, end);

		return node;
	}

	public static List listOfDepths(TNode root) {
	...
	public static void main(String[] args) {
		...
		System.out.println("***================================***");
		// bt.root = getMinimalTree(array, 0, array.length - 1);
		l=listOfDepths(bt.root);
		bt.printStrategy(6);
		System.out.println("***================================***");
		for(LinkedList linkedlist : l){
